...
public class ScriptChunkSizeTest {
    ...
    public static Collection<ScriptChunk> data() {
        ...
        for (int op = 0; op < 0xff; op++) {
            ScriptChunk scriptChunk = new ScriptChunk(op, null);
            if (!scriptChunk.isPushData()) dataless.add(new ScriptChunk(op, null));
        }

        ArrayList<ScriptChunk> smallData = new ArrayList<>(0x4b);
